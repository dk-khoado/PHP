...
<head>
	...
	<script>
		function onSignIn(googleUser) {
			...
			url = "<?php echo base_url() . "main/gg_Login" ?>";
	...
	<script>
		...
		function isSearch() {
			...
			val = $('#search').val()
			url = "<?php echo base_url() . "ApiAjax/Search" ?>";
			var check = <?php if (isset($isSearch)) {
							echo "true";
						} else echo "false";
						?>;
			if (!check) {
				...
				$.ajax({
					...
					$('#searchLoad').children().remove();
					$.each(callback, function(k, v) {
						var data = '<div class="" style="width:220px; float: left">';
						data = data + '<div class="product">';
						data = data + '<div class="product-img">';
						if (v.Image == "") {
							data = data + '<img src="<?php echo base_url() ?>assets/image/default_product.jpg"  height= 150px">';
						} else {
							data = data + '<img src="<?php echo base_url() ?>upload/' + v.Image + '" height= 150px">';
						}
						data = data + '<div class="product-label"></div></div>';
						data = data + '<div class="product-body">';
						data = data + '<p class="product-category">Category</p>';
						data = data + '<h3 class="product-name limitText"><a href=" <?php echo site_url("main/chitiet") ?> ?id=' + v.ID_PRODUCT + '">' + v.NameProduct + '</a></h3>';
						data = data + '<h4 class="product-price">' + formatNumber(v.PriceProduct) + ' <del class="product-old-price"></del></h4>';
						data = data + '<a href=" <?php echo site_url("main/chitiet") ?>?id=' + v.ID_PRODUCT + '" class="btn btn-outline-primary">Chi tiết</a>';
						data = data + '<button onclick="AddCart(' + v.ID_PRODUCT + ')" type="button" class="btn btn-outline-danger"><i class="fa fa-shopping-cart"></i></button>';
						data = data + '</div></div></div>';
						$('#searchLoad').append(data);
					});
					if (callback == "") {
						$('#searchLoad').append("<h3>Không tìm thấy sản phẩm</h3>");
					}
				});
				return false;
			}
	...
	<div class="fixed-top">
				...
				<div class="row">
					...
					<div class="col-sm-12">
						<ul class="social-network">
							<li class="dropdown">
								<button class="btn btn-secondary dropdown-toggle fas fa-shopping-cart" onclick="LoadCart()" data-toggle="dropdown" id="btn_cart">Card</button>
		...
		<nav class="navbar navbar-expand-lg navbar-dark mx-background-top-linear">
			<div class="container">
				...
				<form action="<?php echo site_url("main/Search") ?>" method="POST" onsubmit="return isSearch()">
					<input type="text" value="" id="search" name="tim_kiem" placeholder="Tìm kiếm">
				...
				<div class="collapse navbar-collapse" id="navbarResponsive">
					...
					<ul class="navbar-nav ml-auto">
						...
						if (isset($_SESSION['id']) && isset($_SESSION['name'])) {
							...
							echo '<a class="dropdown-item" onclick="signOut()" href="' . site_url("main/signout") . '">Đăng Xuất</a>';
							echo '</div>';
							echo '</li>';
						} else {
					...
					<div>
						<div class="card">
							...
							<div class="card-footer bg-white p-0">
								...
								<div class="g-signin2" data-onsuccess="onSignIn" data-width="300" data-theme="dark"></div>
							</div>
						</div>
					</div>
				</div>
