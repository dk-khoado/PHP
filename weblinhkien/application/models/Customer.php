...
class Customer extends CI_Model
{
    ...
    function gg_checkLogin($email){
        $query = "custumers where EMAIL = '$email' and type_login = 'google'";
        $result = $this->db->get($query);
        if($result->num_rows() > 0){
            return array("response"=>"true","data" =>$result->row());
        }else{
            return array("response"=>"false","data"=> $result->row());
        }
    }
    function getAll(){
        $result = $this->db->get("custumers");
        return $result->result();
    }
}
